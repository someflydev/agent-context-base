...
def native_smoke_check() -> None:
    if not command_available("gradle"):
        raise unittest.SkipTest("gradle is not available")
    result = run_command(["gradle", "--no-daemon", "classes"], cwd=RUNTIME_DIR, timeout=900)
...
def docker_smoke_check() -> dict[str, object]:
    ...
    tag = f"agent-context-kotlin-http4k-exposed:{free_tcp_port()}"
