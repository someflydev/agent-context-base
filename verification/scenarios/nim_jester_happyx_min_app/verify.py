...
def docker_smoke_check() -> dict[str, object]:
    ...
    tag = f"agent-context-nim-jester-happyx:{free_tcp_port()}"
    build = run_command(["docker", "build", "-t", tag, "."], cwd=RUNTIME_DIR, timeout=900)
    if build.returncode != 0:
        raise AssertionError(build.stderr.strip() or build.stdout.strip())

    port = free_tcp_port()
    run = run_command(
        ["docker", "run", "--rm", "-d", "-p", f"{port}:5000", tag],
        cwd=RUNTIME_DIR,
        timeout=120,
    )
    if run.returncode != 0:
    ...
    try:
        health_status, health_payload = wait_for_http_json(f"http://127.0.0.1:{port}/healthz", timeout=35.0)
