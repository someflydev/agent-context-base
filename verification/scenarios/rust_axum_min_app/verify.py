...
def native_smoke_check() -> None:
    if not command_available("cargo"):
        raise unittest.SkipTest("cargo is not available")
    result = run_command(["cargo", "build"], cwd=RUNTIME_DIR, timeout=300)
...
def docker_smoke_check() -> dict[str, object]:
    ...
    tag = f"agent-context-rust-axum:{free_tcp_port()}"
    ...
    port = free_tcp_port()
    run = run_command(
        ["docker", "run", "--rm", "-d", "-p", f"{port}:3000", tag],
        cwd=RUNTIME_DIR,
        timeout=120,
    )
    if run.returncode != 0:
