...
- `filterRows(state)` — calls `applyTextSearch` first, then `filter { matchesDim(...) }`
  per dimension, then `sortRows`.
- `facetCounts(state, dim)` — calls `applyTextSearch` first; `when (dim)` dispatch; relaxes
  _in, keeps _out for target; applies all other dimensions fully. Sort is never applied.
...
- `applyTextSearch(rows, q)` — filters on `reportId.lowercase().contains(q)`. Returns all
  rows if q is empty.
