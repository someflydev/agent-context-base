defmodule TenantcoreAuth.Registry.RouteRegistry do
  alias TenantcoreAuth.Auth.AuthContext

  def route_registry do
    [
  ...
  def get_allowed_routes(%AuthContext{} = auth) do
    Enum.filter(route_registry(), fn route ->
      cond do
        route[:public] -> true
        route[:super_admin_only] -> auth.tenant_role == "super_admin" and AuthContext.has_permission?(auth, route.permission)
        auth.tenant_role == "super_admin" -> false
        is_nil(route.permission) -> true
        true -> AuthContext.has_permission?(auth, route.permission)
      end
    end)
  end

  def get_guide_sections(auth) do
    auth
    |> get_allowed_routes()
    |> Enum.map(& &1.docs_section)
