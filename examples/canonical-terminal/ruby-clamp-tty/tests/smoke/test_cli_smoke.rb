...
class TaskflowClampCliSmokeTest < Minitest::Test
  def setup
  ...
  def test_list_json
    ...
    assert status.success?, stderr
    assert_equal 20, JSON.parse(stdout).length
  end

  def test_filter
    stdout, stderr, status = run_cli("list", "--status", "failed", "--output", "json")
    assert status.success?, stderr
    payload = JSON.parse(stdout)
    assert payload.all? { |job| job["status"] == "failed" }
  end

  def test_inspect
    stdout, stderr, status = run_cli("inspect", "--output", "json", "job-001")
    assert status.success?, stderr
    assert_equal "job-001", JSON.parse(stdout)["id"]
  end

  def test_stats
    stdout, stderr, status = run_cli("stats", "--output", "json")
    assert status.success?, stderr
    assert_equal 20, JSON.parse(stdout)["total"]
  end
