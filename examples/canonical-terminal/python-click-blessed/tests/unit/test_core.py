...
def test_filter_jobs_by_status() -> None:
    jobs = load_jobs(default_fixtures_path())
    running = filter_jobs(jobs, status="running")
...
def test_filter_jobs_by_tag() -> None:
    jobs = load_jobs(default_fixtures_path())
    deploy = filter_jobs(jobs, tag="deploy")
    assert deploy
    assert all("deploy" in job.tags for job in deploy)
...
def test_load_config_reads_fixture_mode() -> None:
    ...
    assert config["queue_name"] == "taskflow-ci"
