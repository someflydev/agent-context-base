  ...
  module Core
    ...
    module Loader
      ...
      def default_fixtures_path
        ...
        repo_root.join("examples/canonical-terminal/fixtures")
      end
      ...
      def load_events(path = default_fixtures_path)
        load_json(path, "events.json")
          .map { |item| Event.from_h(item) }
          .sort_by(&:timestamp)
      end
      ...
      def load_json(path, filename)
        fixture_dir = Pathname(path).expand_path
        raise FixtureError, "fixtures path does not exist: #{fixture_dir}" unless fixture_dir.exist?

        target = fixture_dir.join(filename)
